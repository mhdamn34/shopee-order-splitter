...
<template>
  <section class="card">
    ...
    <div class="grid grid-cols-[1fr_1fr_32px] items-center gap-1.5 px-0.5 text-[11px] text-muted">
      ...
      <div class="text-right">{{ offLabel }}</div>
      <div></div>
    </div>
    ...
    <div class="rows flex flex-col gap-2">
      ...
      >
        ...
        />
        <button class="del" type="button" title="Remove" @click="emit('remove', index)">×</button>
      </div>

      <p v-if="vouchers.length === 0 && emptyMessage" class="py-3 text-center text-sm text-muted">
        {{ emptyMessage }}
      </p>
    </div>

    <button class="add" type="button" @click="emit('add')">{{ addLabel }}</button>
