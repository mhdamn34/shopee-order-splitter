  ...
  <main class="mx-auto max-w-[560px] md:max-w-[1400px]">
    ...
    >
            ...
            <template #lead>
              <label class="field mb-3 block">
                <span class="mb-1.5 block text-xs text-muted"
                  >Delivery fee per order (RM)</span
                >
                <input
                  v-model="deliveryFee"
                  type="number"
      ...
      <div class="column-output @container/output">
        <p class="status mx-0.5 mb-3.5 text-xs text-muted">{{ status }}</p>

        <template v-if="plan">
          <SplitComparison :rows="comparison" @select="selectOrderCount" />
          <NearMissWarning :near-misses="plan.nearMisses" />
          <PlanView :plan="plan" :delivery-cents="run.deliveryCents" />
